...
class RegisterServiceRequestForm(FlaskForm):
    id_service = SelectField('Servicio', coerce=int, choices=[])
    id_client = SelectField('Cliente', coerce=int, choices=[])
    date_request = DateField('Fecha de solicitud de servicio', validators = [DataRequired()], format='%Y-%m-%d')
    price_request = StringField('Precio', validators = [DataRequired(), Length(min=4, max=25)])
    submit = SubmitField('Registrar')
